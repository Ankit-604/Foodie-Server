...
cartRouter.post("/cart", userAuth, async (req, res) => {
  try {
    ...
    if (!cart) {
      cart = new Cart({ userId, items: [{ cardId, quantity: 1, price }] });
    } else {
...
cartRouter.get("/cart", userAuth, async (req, res) => {
  try {
    ...
    if (!cart || cart.items.length === 0) {
      return res
        .status(200)
        .json({ message: "Cart is empty", cart: [], totalPrice: 0 });
    }
    let totalPrice = 0;
...
cartRouter.delete("/cart/:cardId", userAuth, async (req, res) => {
  try {
    ...
    await cart.save();
    return res.status(200).json({ message: "Item updated in the cart", cart });
  } catch (error) {
    return res
      .status(500)
